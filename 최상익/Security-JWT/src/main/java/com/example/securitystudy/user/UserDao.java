...
public class UserDao {
    ...
    public User insertUser(User user) {
        String insertUserQuery = "insert into user (name,nickname,email,password,role,provider,provider_id, status) values (?,?,?,?,?,?,?,?)";
        Object[] insertUserParams = new Object[]{user.getName(), user.getNickname(), user.getEmail(), user.getPassword(),
                user.getRole(), user.getProvider(), user.getProvider_id(), user.getStatus()};
    ...
    public User selectByEmailProvider(String email, String provider) {
        String selectByEmailQuery = "select id, name, nickname,email, password, introduce, role, provider, provider_id, status from user where email = ? and provider = ? and status = 1";
    ...
    public int checkEmailProvider(String email, String provider) {
        String checkProviderQuery = "select exists(select email from user where email = ? and provider = ? and status = 1)";
        Object[] checkEmailParams = new Object[]{email, provider};
        return this.jdbcTemplate.queryForObject(checkProviderQuery, int.class, checkEmailParams);
    }
